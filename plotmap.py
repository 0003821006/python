...
data = np.loadtxt('level.txt', delimiter=',', comments='#')
...
nx = np.unique(data[:,1]).size # 時間の重複を無くしてから個数を数えてnxとする。
x = data[:,0].reshape(int(data[:,0].size/nx),nx) # 一次元配列をint(data[:,0].size/nx)行nx列の配列に変換する。行が周波数を、列が時間を表す。
y = data[:,1].reshape(int(data[:,1].size/nx),nx)
z = data[:,2].reshape(int(data[:,2].size/nx),nx) # $\mu$ s$^{1/2}$$\cdot$nmに換算
