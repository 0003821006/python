...
data1 = np.loadtxt('q000r000.txt', delimiter=',', comments='#')
filename = 'q000r000wlt.txt'
...
z = data[:,2].reshape(int(data[:,2].size/nx),nx)*1e3 # $\mu$ s$^{1/2}$$\cdot$nmに換算
...
#levels = callevels(z)
fig = plt.figure(figsize=(8,8)) # figsizeを設定
# ax = fig.add_subplot((111)) # figureで設定したエリア全域にaxを設定
ax1 = plt.subplot2grid((5,1),(0,0)) # figure()で確保したエリアを行方向に5分割し、一番上のエリアをax1に割り当てる。これにより、上0.2のエリアに波形をプロットする。
ax = plt.subplot2grid((5,1), (1,0), rowspan=4) # 上から2-5番目のエリアをaxに割り当てる。下0.8にwavelet変換の結果をプロットする。
a = np.average(data1[0:int(np.size(data1[:,0])*0.09),1])
ax1.plot(data1[:,0]*1e6,data1[:,2]-a) # 波形プロット
# ax1.set_title('Sharing both axes') # Title設定
ax1.set_xlim(min(data1[:,0]*1e6),max(data1[:,0]*1e6)) # x軸の範囲の設定
ax1.grid(linestyle='dashed') # gridを破線で表示
...
ax1.set_title(' '+str(date.today())+' '+os.path.join(os.path.basename(os.path.dirname(__file__)),os.path.basename(__file__))+' '+filename,loc='right',fontsize=12)
# plot範囲を正方形にする。
# ax.set_aspect((np.amax(x)-np.amin(x))/(np.amax(y)-np.amin(y)))
#squarify(fig)
# colorbarのlabelの向きは従来の逆
# cbar = fig.colorbar(cs,label='Absolute value of wavelet transformation\nof $u_z$ ($\mu$ s$^{1/2}$$\cdot$V)')
...
# 余白を消してファイルに出力
plt.savefig(filename.split('.')[0]+'w.png', bbox_inches='tight', pad_inches=0)
plt.show()
