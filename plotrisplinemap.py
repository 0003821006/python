...
data = np.loadtxt('../190724/190724-2/x000y003map.txt', delimiter=',', comments='#')
# risplieで作成したmapデータはdataに[時間の数x周波数の数,3]の二次元配列として格納される。
# risplineで作成したmapデータ(*map.txt)は、1列目が時間, 2列目が周波数, 3列目がwavelet変換した結果で、かつ改行のみの行は無視されるので、
# data[:,0]は*map.txtの1列目のデータで時間の1次元配列
# data[:,1]は*map.txtの2列目のデータで周波数の1次元配列
# data[:,2]は*map.txtの3列目のデータでwavelet変換した結果の1次元配列となる。
# これをcontourfで表示するため, x2, y2, z2の2D配列(行が周波数、列が時間)に変換する。
...
z = data[:,2].reshape(int(data[:,2].size/nx),nx)*50
...
#        zmin = 0.0
# 有効数字2桁でzmaxを設定する。
#        iorder =int(np.log10(np.amax(z))) # ゼロでない数字が初めて現れる桁を調べる
#        zmax = np.ceil(np.amax(z)*10**(2-iorder))*10**(iorder-2) # ceilは切り上げ
        return np.linspace(zmin, zmax, num = 21) # 線形に等間隔な数列を生成する
...
ax.set_xlabel('Time $\it{t}$ ($\mu$s)') # x軸のラベル
ax.set_ylabel('Frequency $\it{f}$ (MHz)') # y軸のラベル
ax.set_yscale('log') # y軸はlogスケール
#ax.set_xlim(-0.5,4.5) # x軸の上限と下限
ax.grid(linestyle='dashed') # gridを破線で表示
# plot範囲を正方形にする。
ax.set_aspect((np.amax(x)-np.amin(x))/(np.log10(np.amax(y))-np.log10(np.amin(y))))
# colorbarのlabelの向きは従来の逆
cbar = fig.colorbar(cs,label='Norm of the synthetic wavelet $|d^j_k|$')
...
# 余白を消してファイルに出力
#plt.savefig("result2.png", transparent=True, bbox_inches='tight', pad_inches=0)
plt.show()
